...
-- ------------------------------------------------------------
-- PROCEDURE: add_out_bank
-- Registers an external bank to allow outgoing wire transfers.
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE add_out_bank(
    p_out_bank_id     VARCHAR(255),
    p_out_bank_branch VARCHAR(255)
)
LANGUAGE plpgsql
AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM out_bank_supported WHERE out_bank_id = p_out_bank_id) THEN
        RAISE EXCEPTION 'External Bank ID "%" already exists.', p_out_bank_id;
    END IF;

    INSERT INTO out_bank_supported (out_bank_id, out_bank_branch)
    VALUES (p_out_bank_id, p_out_bank_branch);

    RAISE NOTICE 'External Bank "%" (%) added successfully.', p_out_bank_id, p_out_bank_branch;
END;
$$;

-- ------------------------------------------------------------
-- PROCEDURE: add_bill_type
-- Registers a new utility/service for the bill payment system.
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE add_bill_type(
    p_bill_type VARCHAR(255)
)
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO bill_type_supported (bill_type)
    VALUES (p_bill_type);

    RAISE NOTICE 'Bill type "%" added successfully.', p_bill_type;
END;
$$;

CREATE OR REPLACE PROCEDURE add_bill_provider(
...
AS $$
BEGIN
    -- Guard: Ensure the bill category type actually exists first
    IF NOT EXISTS (SELECT 1 FROM bill_type_supported WHERE bill_id = p_bill_id) THEN
        RAISE EXCEPTION 'Bill Category ID % does not exist.', p_bill_id;
    END IF;

    INSERT INTO bill_provider_supported (bill_provider_name, bill_id)
    VALUES (p_provider_name, p_bill_id);

    RAISE NOTICE 'Provider "%" added successfully under Bill Category ID %.', p_provider_name, p_bill_id;
END;
$$;
...
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_out_bank_transaction(
...
BEGIN
    ...
    IF NOT FOUND OR v_source_status = 'closed' THEN
        RAISE EXCEPTION 'Source account is invalid or closed.';
    END IF;
    ...
    IF NOT v_bank_exists THEN
        RAISE EXCEPTION 'Destination bank ID % is not supported.', p_destination_bank_id;
    END IF;
...
BEGIN
    ...
    IF NOT FOUND OR v_source_status = 'closed' THEN
        RAISE EXCEPTION 'Account is invalid or closed.';
    END IF;

    IF v_source_balance < p_amount THEN
        RAISE EXCEPTION 'Insufficient funds to pay bill.';
    END IF;

    -- 3. Derive Bill Type and Provider Name in 3NF
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Provider ID % does not exist.', p_bill_provider_id;
    END IF;
