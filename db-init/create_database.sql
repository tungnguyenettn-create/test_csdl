...
-- No FK on destination_bank_id — the destination is outside this system.
...
CREATE TABLE out_bank_supported (
    out_bank_id VARCHAR(255) PRIMARY KEY, 
    out_bank_branch VARCHAR(255) NOT NULL UNIQUE
);

ALTER TABLE out_bank_trans 
    ADD CONSTRAINT fk_out_bank_branch 
    FOREIGN KEY (destination_bank_id) 
    REFERENCES out_bank_supported (out_bank_id); 
-- ------------------------------------------------------------
-- TABLE: bill
-- Extends trans for bill payments (utilities, phone, insurance).
--   affected_account_id (in trans) = the account paying the bill
-- ------------------------------------------------------------
CREATE TABLE bill_type_supported (
    bill_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, 
    bill_type VARCHAR(255) NOT NULL
);

CREATE TABLE bill_provider_supported (
    bill_provider_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,  
    bill_provider_name VARCHAR(255) NOT NULL, 
    bill_id INT
); 

ALTER TABLE bill_provider_supported 
    ADD CONSTRAINT fk_bill_provider_type
    FOREIGN KEY (bill_id) 
    REFERENCES bill_type_supported (bill_id); 
...
ALTER TABLE bill 
    ADD CONSTRAINT fk_bill_provider 
    FOREIGN KEY (bill_provider_id) 
    REFERENCES bill_provider_supported (bill_provider_id);
